...
// FIX: Added DISTINCT ON (p.id) to prevent duplicate rows when a product has
// multiple pricing_rules entries — the LEFT JOIN was returning one row per rule.
  ...
  try {
    ...
    if (status && status !== 'All') {
      const statusMap = {
        ...
        'Low Stock': `i.stock < i.reorder_point AND i.stock > 10`,
        'Critical':  `i.stock <= 10`,
      };
    ...
    const result = await pool.query(
      `SELECT DISTINCT ON (p.id)
              p.sku AS id, p.name, p.category, i.stock, i.reorder_point AS reorder,
       ...
       ${where}
       ORDER BY p.id, pr.updated_at DESC`,
      values
    );
...
// FIX: Added DISTINCT ON (p.id) + LIMIT per product to prevent duplicate chart bars
// when demand_forecast has multiple rows for the same product.
router.get('/demand-forecast', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT DISTINCT ON (p.id)
              p.name AS product, df.current_qty AS current, df.predicted_qty AS predicted
       FROM demand_forecast df
       JOIN products p ON p.id = df.product_id
       ORDER BY p.id, df.forecast_date DESC`
    );
    res.json(result.rows);
  } catch (err) {
    console.error(err);
